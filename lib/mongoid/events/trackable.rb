      ...
      def modified_attributes_for_update
        @modified_attributes_for_update ||= if events_trackable_options[:on] == :all
                                              changes_with_relations.reject do |k, v|
                                                events_trackable_options[:except].include?(k) or ( is_number?(v[0]) && is_number?(v[1]) && ( Float(v[0]) == Float(v[1]) ) )
                                              end
                                            else
                                              changes_with_relations.reject do |k, v|
                                                !events_trackable_options[:on].include?(k) or ( is_number?(v[0]) && is_number?(v[1]) && ( Float(v[0]) == Float(v[1]) ) )
                                              end
